...
export class Backup {
  ...
  async execute() {
    const trashed = path.join(os.tmpdir(), path.basename(this.backupDir) + Date.now());
    try {
      await rename(this.backupDir, trashed);
      console.info(colors.green(`Moved old ${this.backupDir} to ${trashed}`));
    } catch (err) {
      if ('ENOENT' !== err.code) {
        throw err;
      }
    ...
    const initialFolder = '/' === this.category.slice(-1) ? this.category : this.category + '/';
    this.enqueue(initialFolder);
    if (this.includePublic && this.category) {
  ...
  async checkFetch() {
    ...
    if (nextPath) {
      if (numInFlight < this.simultaneous) {
        ...
        console.warn(colors.yellow("connections are maxed-out"));
      }
    } else {
  ...
  async fetchItem(rsPath, handle) {
    try {
      ...
      const res = await fetch(new URL(encodePath(rsPath), this.storageEndpoint), fetchOptions);
      ...
      switch (res.status) {
        ...
        case 503:
          ...
          console.warn(colors.yellow(`${res.status}${res.statusText ? " " + res.statusText : ""}: pausing for ${retryAfter}ms ${rsPath}`))
          break;
        case 401:
        case 403:
        ...
        case 404:
        case 410:
          console.error(colors.red(`${res.status}${res.statusText ? " " + res.statusText : ""} ${rsPath} doesn't exist any more`));
          this.dequeue(rsPath);
          break;
        default:
